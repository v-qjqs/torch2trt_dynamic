...
size_t TorchNMSPluginDynamic::getWorkspaceSize(
  ...
  size_t index_workspace =
      getAlignedSize(num_bbox * sizeof(int));
  switch (data_type) {
  ...
  tmp_score_workspace = getAlignedSize(tmp_score_workspace);
  final_score_workspace =
      getAlignedSize(final_score_workspace);
  nms_workspace = getAlignedSize(nms_workspace);
